...
__device__ inline void test1(FileSystem *fs, uchar *input, uchar *output) {
    /////////////// Test Case 1  ///////////////
    printf("[Opening t.txt]\n");
	u32 fp = fs_open(fs, "t.txt\0", G_WRITE);

	printf("[Writing t.txt]\n");
	fs_write(fs, input, 64, fp);
	
	printf("[Opening b.txt]\n");
	fp = fs_open(fs, "b.txt\0", G_WRITE);
	
	printf("[Writing b.txt]\n");
...
__device__ void test2(FileSystem *fs, uchar *input, uchar *output) {
    /////////////// Test Case 2  ///////////////
    printf("[Opening t.txt]\n");
	u32 fp = fs_open(fs, "t.txt\0", G_WRITE);
...
__device__ void test3(FileSystem *fs, uchar *input, uchar *output) {
    /////////////// Test Case 3  ///////////////
	u32 fp = fs_open(fs, "t.txt\0", G_WRITE);
	fs_write(fs, input, 64, fp);
	fp = fs_open(fs, "b.txt\0", G_WRITE);
...
__device__ void test4(FileSystem *fs, uchar *input, uchar *output) {
    /////////////// Test Case 4  ///////////////
    ...
    for (int j = 0; j < 1023; ++j) {
		...
		// if (j == 1022) printf("[Test 4] opening %s\n", tag);
        fp = fs_open(fs, tag, G_WRITE);
        // if (j == 1022) printf("[Test 4] writing %s\n", tag);
        fs_write(fs, input + j * 1024, 1024, fp);
    }
	// printf("[Test 4] num files: %d\n", fs->num_files);
    fs_gsys(fs, RM, "32-block-0");
    // printf("[Test 4] num files: %d\n", fs->num_files);
	fs_gsys(fs, LS_D); // added
