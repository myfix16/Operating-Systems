...
__device__ void clear_virtual_memory(VirtualMemory* vm);

__device__ inline void check_valid_tid(VirtualMemory* vm) {
    const u32 tid = threadIdx.x;
    if (*vm->current_tid != tid) {
        clear_virtual_memory(vm);
        *vm->current_tid = tid;
    }
}
...
__device__ void pte_set(pte_t& pte, const bool valid, const u32 next_lru, const u32 vpn) {
...
__device__ void vm_init(VirtualMemory* vm, uchar* buffer, uchar* storage, pte_t* invert_page_table,
                        ...
                        int physical_mem_size, int storage_size, int page_entries, u32* tid) {
