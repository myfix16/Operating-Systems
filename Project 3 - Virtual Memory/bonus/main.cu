...
__device__ __managed__ uchar storage_charmap[VIRTUAL_PAGE_NUM];

__device__ __managed__ u32 tid;


__device__ void user_program(VirtualMemory* vm, uchar* input, uchar* results, int input_size);

__global__ void mykernel(int input_size, Lock thread_lock) {
    // memory allocation for virtual_memory
    ...
    vm_init(&vm, data, storage, pt, swap_table, storage_charmap, swap_buffer, &pagefault_num,
            PAGE_SIZE, INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE, PHYSICAL_PAGE_NUM, &tid);
...
int main() {
    ...
    mykernel<<<1, 4, INVERT_PAGE_TABLE_SIZE>>>(input_size, thread_lock);
